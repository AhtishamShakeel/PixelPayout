...
object AdCadence {
    ...
    /**
     * Break points outside games and quizzes where an interstitial may play.
     * Each is a moment the player has just FINISHED something - never on the
     * way into Wallet, a redemption, Profile, Help or sign-in.
     */
    enum class Moment {
        /** Closed the daily login / daily goal popup without doubling. */
        CLAIM_POPUP_CLOSED,
        /** Left Level rewards after claiming at least one reward there. */
        LEVEL_REWARDS_EXIT,
        /** Left the weekly Leaderboard. */
        LEADERBOARD_EXIT,
        /** Switched to Home, Play or Earn. Longest gap of all - a safety net. */
        TAB_SWITCH
    }

    /**
     * Interstitials actually shown this session, for the per-session cap.
     * In memory on purpose: a fresh process is a fresh session. A long idle
     * gap also starts a new one - see [sessionCount].
     */
    private var sessionShown = 0
    private var sessionLastShownAt = 0L
    private const val SESSION_IDLE_RESET_MS = 30 * 60 * 1000L

    /** Level rewards arms its exit moment on a successful claim. */
    @Volatile private var levelRewardsClaimed = false
    ...
    fun noteInterstitialShown(context: Context) {
        sessionShown = sessionCount() + 1
        sessionLastShownAt = System.currentTimeMillis()
        context.applicationContext
            .getSharedPreferences(PREFS, Context.MODE_PRIVATE)
            .edit()
    ...
    /** Called by Level rewards when a claim succeeds on that screen. */
    fun noteLevelRewardClaimed() {
        levelRewardsClaimed = true
    }

    /**
     * Level rewards was left for somewhere no ad may play (Wallet, Profile).
     * The claim belongs to that visit, so it must not arm a later exit.
     */
    fun forgetLevelRewardClaim() {
        levelRewardsClaimed = false
    }

    /**
     * Whether an interstitial should play at [moment].
     *
     * Stricter than [onActivityCompleted], because these interrupt browsing
     * rather than follow a finished run:
     *
     *   * nothing until the new-install grace period is over (3 finished
     *     games or quizzes), so a new player's first look at the app is clean;
     *   * never within 45s of a rewarded ad;
     *   * a longer gap since the last interstitial of ANY kind - `momentGapSeconds`,
     *     or `tabSwitchGapSeconds` for tab switches (config/ads);
     *   * the per-session cap, shared with games and quizzes.
     *
     * A shown ad resets the game/quiz counter too (noteInterstitialShown), so
     * a moment ad and a completion ad can never stack.
     */
    fun onMoment(context: Context, moment: Moment): Boolean {
        if (moment == Moment.LEVEL_REWARDS_EXIT) {
            if (!levelRewardsClaimed) return false
            levelRewardsClaimed = false
        }
        ...
        if (prefs.getInt(KEY_LIFETIME, 0) < GRACE_COMPLETIONS) return false
        if (watchedRewardedRecently(prefs)) return false
        if (sessionCapReached()) return false

        val gap = if (moment == Moment.TAB_SWITCH) {
            AdNetworkConfigStore.tabSwitchGapMs
        } else {
            AdNetworkConfigStore.momentGapMs
        }
        val lastShownAt = prefs.getLong(KEY_LAST_SHOWN_AT, 0L)
        val elapsed = System.currentTimeMillis() - lastShownAt
        if (lastShownAt != 0L && elapsed in 0 until gap) return false

        if (!InterstitialAdManager.getInstance().canServeWithin(HOLD_WINDOW_MS)) return false

        Log.d(TAG, "Interstitial due at $moment")
        return true
    }

    private fun sessionCount(): Int {
        val idle = System.currentTimeMillis() - sessionLastShownAt
        if (sessionLastShownAt != 0L && (idle < 0 || idle > SESSION_IDLE_RESET_MS)) {
            sessionShown = 0
        }
        return sessionShown
    }

    private fun sessionCapReached(): Boolean {
        val cap = AdNetworkConfigStore.maxInterstitialsPerSession
        val reached = sessionCount() >= cap
        if (reached) Log.d(TAG, "Session cap of $cap interstitials reached")
        return reached
    }
}
