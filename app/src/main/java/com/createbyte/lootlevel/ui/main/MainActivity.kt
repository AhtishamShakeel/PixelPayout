...
import com.createbyte.lootlevel.databinding.ActivityMainBinding
import com.createbyte.lootlevel.ui.dialogs.ReferralDialogFragment
import com.createbyte.lootlevel.ui.quiz.QuizListViewModel
...
import com.createbyte.lootlevel.utils.showLeaderboardPrize
import com.createbyte.lootlevel.utils.showPendingLevelRewards
import com.createbyte.lootlevel.utils.showRedemptionResult
import com.createbyte.lootlevel.ui.dialogs.NoInternetDialog
...
class MainActivity : AppCompatActivity() {
    ...
    private fun setupNavigation() {
        ...
        binding.bottomNav.setOnItemSelectedListener { itemId ->
            ...
            // A tap landing while an ad hold is up is ignored, not queued.
            if (AdHold.isActive) return@setOnItemSelectedListener false
            ...
            if (interstitialDueBefore(itemId)) {
                // The ad first, THEN the tab - so it sits between the two
                // screens rather than over the one just opened. The bar
                // follows once the navigation actually happens.
                AdHold.showInterstitialThen(this) { go() }
                false
            } else {
                go()
                true
    ...
    /**
     * Leaves Leaderboard or Level rewards - their back button and the system
     * back gesture both come here - with an interstitial first if one is due.
     *
     * The ad plays BEFORE the pop, so it lands between the two screens.
     * AdMob disallows an interstitial that appears just after new content
     * has loaded, which is what showing it on arrival would do.
     */
    fun popBackWithBreak() {
        if (AdHold.isActive) return
        val navController = (supportFragmentManager
            .findFragmentById(R.id.nav_host_fragment) as? NavHostFragment)
            ?.navController ?: return
        val to = navController.previousBackStackEntry?.destination?.id ?: 0

        if (interstitialDueBefore(to)) {
            AdHold.showInterstitialThen(this) { navController.popBackStack() }
        } else {
    ...
    /**
     * Whether an interstitial should play before navigating to [to] - see
     * AdCadence.Moment. Asked BEFORE the navigation, never after it.
     *
     * Only ever on the way INTO Home, Play or Earn. Wallet (redemptions),
     * Profile, Help and everything under them are never interrupted, however
     * the player gets there.
     */
    private fun interstitialDueBefore(to: Int): Boolean {
        val navController = (supportFragmentManager
            .findFragmentById(R.id.nav_host_fragment) as? NavHostFragment)
            ?.navController ?: return false
        val from = navController.currentDestination?.id ?: return false
        if (from == to) return false

        if (to !in AD_SAFE_DESTINATIONS) {
            // A claim belongs to that visit; it must not arm a later exit.
            if (from == R.id.levelRewardsFragment) AdCadence.forgetLevelRewardClaim()
            return false
        }

        val moment = when (from) {
            R.id.levelRewardsFragment -> AdCadence.Moment.LEVEL_REWARDS_EXIT
            R.id.leaderboardFragment -> AdCadence.Moment.LEADERBOARD_EXIT
            in TAB_DESTINATIONS -> AdCadence.Moment.TAB_SWITCH
            else -> return false
        }
        return AdCadence.onMoment(this, moment)
    }
    ...
    companion object {
        ...
        /** Bottom-bar tabs; switching between them is the TAB_SWITCH moment. */
        private val TAB_DESTINATIONS = setOf(
            R.id.navigation_home, R.id.navigation_play, R.id.navigation_rewards,
            R.id.navigation_redemption, R.id.navigation_profile
        )
