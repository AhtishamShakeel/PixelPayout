...
import com.createbyte.lootlevel.databinding.FragmentLevelRewardsBinding
import com.createbyte.lootlevel.databinding.ViewLevelRewardsFooterBinding
import com.createbyte.lootlevel.databinding.ViewLevelRewardsHeaderBinding
...
class LevelRewardsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // Back button and back gesture both leave through MainActivity, which
        // may put an interstitial between this screen and the next.
        requireActivity().onBackPressedDispatcher.addCallback(viewLifecycleOwner) { leave() }
    ...
    private fun bindHeader(header: ViewLevelRewardsHeaderBinding) {
        header.levelRewardsBack.setOnClickListener { leave() }
     ...
     */
    private fun leave() {
        (activity as? MainActivity)?.popBackWithBreak() ?: findNavController().popBackStack()
    }
    ...
    private fun claim() {
        ...
        requireActivity().lifecycleScope.launch {
            ...
            claimInFlight = false
            if (result is UserRepository.LevelRewardResult.Claimed) {
                AdCadence.noteLevelRewardClaimed()
            }
